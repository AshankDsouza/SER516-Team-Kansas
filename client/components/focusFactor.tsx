...
async function FocusFactor(props: any) {
  ...
  const response1 = await fetch(`http://localhost:8080/api/${sprints[0].id}/getTotalPoint`, requestOptions)
  const response2 = await fetch(`http://localhost:8080/api/${sprints[0].id}/getCompletedPoints`, requestOptions)
  let lab = [];
  let ser = [];
