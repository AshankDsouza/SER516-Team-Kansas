...
# dummy api to check if the service is running
@app.route("/work", methods=["GET"])
def work():
    return "Work AUC Microservice is running"


# add route with project_slug and auth_token as parameters in the URL
@app.route("/work-auc-data/<project_slug>/<auth_token>", methods=["GET"])
def get_work_auc_data_by_slug(project_slug, auth_token):
    try:
        project = get_project_by_slug(project_slug, auth_token)
        sprintMapping, sprints = get_number_of_milestones(
            project["id"], auth_token
        )
        ...
        for sprint_id in list(sprintMapping.values()):
            milestone = get_milestones_by_sprint(
                project_id, sprint_id, auth_token
            )
